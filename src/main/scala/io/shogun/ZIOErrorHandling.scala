...
import zio.{Cause, Task, UIO, URIO, ZIO, ZIOAppDefault}
...
object ZIOErrorHandling extends ZIOAppDefault {
  ...
  val handleBoth: ZIO[Any, Nothing, String] = anAttempt.fold(
    failure = ex => s"Something bad happened: ${ex}",
    success = value => s"Length ${value}"
  )
  // effectful fold: FoldZIO
  val handleBoth_v2: ZIO[Any, Nothing, String] = anAttempt.foldZIO(
    failure = ex => ZIO.succeed(s"Something bad happened: ${ex}"),
    success = value => ZIO.succeed(s"Length ${value}")
  )
   ...
   */
  // My Answer
  def fromTry[R](maybe: Try[R]): ZIO[Any, Throwable, R] = {
    maybe match {
  ...
  def fromOption[R](maybe: Option[R]): ZIO[Any, Throwable, R] = {
    maybe match {
  ...
  def fromEither[R](maybe: Either[Throwable, R]): ZIO[Any, Throwable, R] = {
    maybe match {
  ...
  def either(effect: ZIO[Any, Throwable, Int]): URIO[Any, Either[Throwable, Int]] =
    effect.foldZIO(
      failure = ex => ZIO.succeed(Left(ex)),
      success = value => ZIO.succeed(Right(value))
    )

  def absolve(urio: URIO[Any, Either[Throwable, Int]]): ZIO[Any, Throwable, Int] =
    ???
  //    urio.foldZIO(
  //      success = (maybe: Either[Throwable, Int]) => maybe match {
  //        case Right(value) => ZIO.succeed(value)
  //        case Left(ex) => ZIO.fail(ex)
  //      }
  //    )
  ...
  /*
    Solutions:
    - design an error model (ex: AppError)
  ...
  //    .foldCauseZIO(
  //      error => ZIO.fail(new RuntimeException(s"error, message: ${error.defects}")),
  ...
  //    zio.foldCauseZIO(
  //      error => ZIO.fail(Left(error.failureOption.orElse(new RuntimeException()))),
  //      value => value match {
  //        case Right(res) => ZIO.succeed(res)
  //        case Left(error) => ZIO.fail(Left(error))
  //      }
  //    )
  //
  //  // 4
  ...
  //        case None => ZIO.fail(QueryError("not found user"))
  //      }
  ...
  override def run = ioException(aBadFailure)
}